...
type
  ...
  strict private
    ...
    (*
      Nueva funcion, presionar Ctrl + F y buscar donde se implementa para ver
      los cambios
    *)
    function OrderNotifications(AStr: string): string;
  public
    constructor Create;
...
begin
  ...
  try
    ...
    Notifications.Text := OrderNotifications(JStringToString(NotificationsStr));
    Found := False;
    I := 0;
...
begin
  ...
  begin
    ...
    try
      NotificationsList.Text := OrderNotifications(JStringToString(Notifications));
...
function TAndroidPreferenceAdapter.OrderNotifications(AStr: string): string;
var
  S1, S2: string;
  i: Integer;
  C: Char;
...
begin
  (*
    Descripcion:
    Al obtener la lista de notificaciones pendientes, la devolucion es un string
    con la siguiente estructura:
      noti2=1noti3=2noti4=3noti2=61noti2=65noti1=76noti2=77noti3=78noti4=79
      noti1=140
      noti2=141
      noti3=142

    Y viendo el codigo, para obtener el nombre o el identificador de cada notificacion
    se recorre un TStringList donde se espera que en cada linea se tenga una notificacion con su
    identificador por ejemplo:
      noti1=1
      noti2=2

    Pues eso es lo que hace esta funcion, distribuye de manera correcta las notificaciones
    para que al momento de pasar el result al TStringList, este tenga la estructura correcta.
  *)
  S1 := AStr.Replace(sLineBreak, string.Empty).Trim; //Se deja una sola cadena de caracteres sin saltos de linea
  S2 := string.Empty;
  EqualCount:= 0;
  ...
  begin
    if S1[i] = '=' then
      Inc(EqualCount); //Se toma el número de "=" como contador de control
  end;

  while (Pos('=', S1) > 0) and (EqualCount > 0) do
  begin
    if EqualCount = 1 then
    begin
      S2:= S2 + S1.Trim + sLineBreak; //Al haber una sola notificacion restante solamente se concatena
      //EqualCount:= 0;
      Break;
    end else
    begin
      for i:= Pos('=', S1) + 1 to Length(S1) do //Desde el caracter seguiente de "=" hasta el final del string
      begin
        C:= S1[i];
        //Si no es un número significa que es el primer caracter del nombre de otra notificacion
        if not TryStrToInt(C, Number) then
        begin
          S2:= S2 + Copy(S1, 1, i - 1) + sLineBreak; //La notificacion detectada se agrega a la lista ordenada
          S1:= S1.Remove(1, i - 1); //Se remueve de la lista de notificaciones no ordenadas
          Dec(EqualCount); //Restar 1 al contador de notificaciones por cada ordenamiento
          Break;
        end;
      end;
    end;
  ...
  Result:= S2.Trim; //El string S2 contiene la lista ordenada de notificaciones
end;
